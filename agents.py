...
root_agent = LlmAgent(
    ...
    description=(
        ...
        "powered by Azure OpenAI via LiteLLM."
    ),
    instruction=(
        "I can answer your questions about the time and weather in major cities. "
        "I support New York, London, Tokyo, Los Angeles, Paris, and Sydney. "
        "Just ask me about the weather or time in any of these cities!"
    ),
    tools=[get_weather, get_current_time, get_agent_info],
)

if __name__ == "__main__":
    print("Weather and Time Agent initialized successfully!")
    print(f"Using model: {AZURE_OPENAI_MODEL}")
    print(f"Azure endpoint: {AZURE_OPENAI_ENDPOINT}")
    print("Run 'adk web' to start the web interface.")
